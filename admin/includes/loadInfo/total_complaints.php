...
$sel = mysqli_query($con, "select count(*) as totalcount from complaints c where 1");

$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['totalcount'];

## Total number of record with filtering
$sel = mysqli_query($con, "select count(*) as totalcountfilters from complaints c WHERE 1 and " . $searchQuery . "&& (" . $filterQuery . ")");
...
$sql ="select RequestID,BlockNumber,FlatNumber,complaint_type,Description,RaisedDate,AdminRemark,Status,ResolvedDate,updated_at FROM complaints inner join complainttypes on complainttypes.complaint_id = complaints.ComplaintType WHERE 1 and "
